...
/* Official AZ-104 "Skills at a glance" sections. Every question carries a
   "domain" (1-3). */
const DOMAINS = {
  1: { num: "1", name: "Manage Azure identities and governance", weight: "20–25%" },
  2: { num: "2", name: "Implement and manage storage", weight: "15–20%" },
  3: { num: "3", name: "Deploy and manage Azure compute resources", weight: "20–25%" },
  4: { num: "4", name: "Implement and manage virtual networking", weight: "15–20%" },
  5: { num: "5", name: "Monitor and maintain Azure resources", weight: "10–15%" },
};
...
async function loadQuestions() {
  try {
    ...
    const defaultSize = Math.min(50, state.questionPool.length);
    ...
    els.loadingMsg.textContent =
      "Could not load questions.json — serve this folder over HTTP (e.g. python3 -m http.server).";
...
function openFlashSetup() {
  ...
  els.flashSetupLead.innerHTML =
    ...
    `AZ-104 questions. Read the question, recall the answer, then reveal and grade yourself.`;
...
function startExam() {
  const size = parseInt(els.quizSize.value, 10) || 50;
  ...
  for (const d of [1, 2, 3]) {
    ...
    row.innerHTML = `
      <div class="section-row-head">
        ...
        <span class="section-row-name">${escapeHtml(DOMAINS[d].name)} <span class="muted">(${escapeHtml(DOMAINS[d].weight)} of the exam)</span></span>
...
function examVerdict(pct) {
  if (pct >= 90) return "Outstanding — you're well above the AZ-104 passing range.";
  if (pct >= 80) return "Strong work — keep drilling weak topics and you're set.";
  if (pct >= 70) return "Right around the AZ-104 pass line (700/1000). Review your misses below.";
...
function showFlashResults() {
  ...
  els.resultsHeadline.innerHTML = `You knew <span>${score}</span> of <span>${total}</span> questions.`;
  els.resultPercent.textContent = `${pct}%`;
